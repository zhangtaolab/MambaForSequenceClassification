...
class Mamba2ForSequenceClassification(Mamba2PreTrainedModel):
    ...
    @replace_return_docstrings(output_type=Mamba2SequenceClassifierOutput, config_class=_CONFIG_FOR_DOC)
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=Mamba2SequenceClassifierOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        use_cache: Optional[bool] = None,
        **kwargs,
    ) -> Union[Mamba2SequenceClassifierOutput, Tuple[torch.FloatTensor]]:
        r"""
        ...
        last_hidden_states = mamba2_outputs[0]
        ...
        if not return_dict:
            output = (pooled_logits,) + mamba2_outputs[1:]
            return ((loss,) + output) if loss is not None else output

        return Mamba2SequenceClassifierOutput(
            loss=loss,
